...
/**
 * reverse coordinates next to given square center
 * @param  {[type]} square [description]
 * @param  {[type]} params [description]
 * @return {[type]}        [description]
 */
const nextTo = (square, params) => new Promise((resolve, reject) => {
  const center = {
...
const around = params => new Promise((resolve, reject) => {
  geocode(params).then((data) => {
    nextTo(data, params).then((response) => {
      resolve(response);
    }).catch((err) => {
      reject(err);
    });
